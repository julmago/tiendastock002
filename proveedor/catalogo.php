  ...
  if (($p['status'] ?? '') !== 'active') $err="Cuenta pendiente de aprobación.";
    ...
    elseif ($sku !== '' && !preg_match('/^\d{8,14}$/', $sku)) $err = "El código universal debe tener entre 8 y 14 números.";
      ...
      if ($product_id > 0) {
        ...
        if ($st->fetch()) {
          $pdo->prepare("UPDATE provider_products SET title=?, sku=?, description=?, base_price=? WHERE id=? AND provider_id=?")
              ->execute([$title,$sku?:null,$desc?:null,$price,$product_id,(int)$p['id']]);
          $msg="Actualizado.";
          $edit_id = $product_id;
        } else {
...
if ($edit_id > 0) {
  $st = $pdo->prepare("SELECT id,title,sku,description,base_price FROM provider_products WHERE id=? AND provider_id=? LIMIT 1");
